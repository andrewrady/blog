...
Developing rails applications on windows has always been a pain. It's simple enough to install ruby and the need gems to get a basic Rails application started, but in time developers will run into problems. A lot of gem work fine in a windows environment, but some do not compile down correctly on this is the main issue with developing on Windows. For this reason many people who need to work on rails applications are running their development environment on OSX or Linux including me. With Windows WSL I decided to see if I can get a working, acceptable development environment on a Windows computer. After some trail and error (plus some help from google) it's pretty good.

## Windows Subsystem for Linux (WSL)

Microsoft now lets us install Linux distro system directly on our windows computer easily from the Microsoft Store. Before we do this though you need to open Powershell as an Admin run
```
Enable-WindowsOptionalFeature -Online -FeatureName Microsoft-Windows-Subsystem-Linux
```
and then restart our computer. The easiest way to install a Linux distro is from the Windows Store. Just open it up and search for your preferred distro. Personally I am using Ubuntu because I have the most experience with that one. For a more detail on installing WSL on windows [check out this article](https://docs.microsoft.com/en-us/windows/wsl/install-win10). If we search for Ubuntu from the windows search it will open a terminal window that we can everything up.

## Setting up our Environment

For now we are going to do a simple install for talk about a few gotchas developing with WSL. Normally I would recommend installing Ruby using RVM so you can easily change versions if need be. For now lets just install of ruby. First update our new system and then install ruby 
...
Now we need to install lets install Bundler which is a package manager similar to npm for node development. We can do this with
```
gem install bundler
```
...
If you are a vim user you are good, but us new kids to the block who uses flashy text editors then there is one small probably. Window doesn't want you going into the WSL and manually editing files. This can causes us problems if you use something like vscode. After some research Microsoft has setup a directory to work on project and be able to open them on the Windows side and work like normal. In the WSL terminal we can get to this by,
```
cd /mnt/c/dev/
```
